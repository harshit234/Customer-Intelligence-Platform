...
def business_reading(
    ...
    lines = [
        ...
        f"    Call cost  ({n_contacted:,} x ${cost_per_call:.0f})  : "
        f"${total_cost:,.0f}",
        f"    Revenue    ({est_subscribers:,} x ${revenue_per_subscription:.0f}): "
        f"${total_revenue:,.0f}",
        f"    Net value                : ${net:,.0f}",
        "=" * 60,
    ]
...
class PromotionGate:
    ...
    def compare(
        ...
        if improved_latency_ms is not None:
            ...
            details.append("[SKIP] Latency check - no measurement provided")

        if failures:
            return GateResult(
                promoted=True,
                reason=f"Improved blocked ({len(failures)} check(s) failed) - baseline promoted.",
                winner="baseline",
        ...
        return GateResult(
            promoted=True,
            reason="All 3 gate checks passed - improved model promoted.",
            winner="improved",
            challenger_metrics=improved_metrics,
            champion_metrics=baseline_metrics,
            gate_details=details,
    ...
    def evaluate(
        ...
        if failures:
            return GateResult(
                promoted=False,
                reason=f"Absolute gate failed - {len(failures)} check(s) not met.",
        ...
        if champion_metrics:
            ...
            if champ_val is not None and chal_val is not None:
                ...
                details.append(
                    ...
                    f"delta={delta:+.4f}  required>={required:.4f}"
                )
                if not ok:
                    return GateResult(
                        promoted=False,
                ...
                details.append(
                    f"[SKIP] relative gate - {self.primary_metric} not available in champion metrics"
                )
        else:
            details.append("[SKIP] relative gate - no champion metrics provided (first run)")
